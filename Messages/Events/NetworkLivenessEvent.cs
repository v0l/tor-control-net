...
namespace tor_control_net.Messages.Events;
...
public record NetworkLivenessEvent : IAsyncEvent
{
    ...
    public static NetworkLivenessEvent FromReply(TorControlReply reply)
    {
        ...
        {
            throw new TorControlReplyParseException(
                $"{nameof(NetworkLivenessEvent)}: Expected {StatusCode.AsynchronousEventNotify} status code.");
        }
